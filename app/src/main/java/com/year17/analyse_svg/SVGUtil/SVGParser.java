...
public class SVGParser {
    ...
    static SVG parse(InputSource data, SVGParser.SVGHandler handler)
    ...
    private static SVGParser.NumberParse parseNumbers(String s) {
        ...
        for (int i = 1; i < n; i++) {
            ...
            switch (c) {
                ...
                case ')': {
                    ...
                    p = i;
                    return new SVGParser.NumberParse(numbers, p);
                }
                case '-':
    ...
    private static Matrix parseTransformItem(String s, Matrix matrix) {
        if (s.startsWith("matrix(")) {
            SVGParser.NumberParse np = parseNumbers(s.substring("matrix(".length()));
            ...
            SVGParser.NumberParse np = parseNumbers(s.substring("translate(".length()));
            ...
            SVGParser.NumberParse np = parseNumbers(s.substring("scale(".length()));
            if (np.numbers.size() > 0) {
            ...
            SVGParser.NumberParse np = parseNumbers(s.substring("skewX(".length()));
            if (np.numbers.size() > 0) {
                float angle = np.numbers.get(0);
                matrix.preSkew((float) Math.tan(angle), 0);
            }
        } else if (s.startsWith("skewY(")) {
            SVGParser.NumberParse np = parseNumbers(s.substring("skewY(".length()));
            if (np.numbers.size() > 0) {
                float angle = np.numbers.get(0);
                matrix.preSkew(0, (float) Math.tan(angle));
            }
        } else if (s.startsWith("rotate(")) {
            SVGParser.NumberParse np = parseNumbers(s.substring("rotate(".length()));
            if (np.numbers.size() > 0) {
                float angle = np.numbers.get(0);
                float cx = 0;
                float cy = 0;
...
//                        * ((largeArc == sweepArc) ? -1 : 1);
    ...
    private static SVGParser.NumberParse getNumberParseAttr(String name, Attributes attributes) {
    ...
    private static class Gradient {
        ...
        */
        public void inherit(SVGParser.Gradient parent) {
            SVGParser.Gradient child = this;
    ...
    private static class Properties {
        SVGParser.StyleSet styles = null;
        Attributes atts;
        ...
        private Properties(Attributes atts) {
            ...
            if (styleAttr != null) {
                styles = new SVGParser.StyleSet(styleAttr);
            }
        }
    ...
    static class SVGHandler extends DefaultHandler {
        ...
        SVGParser.Gradient gradient = null;
        ...
        private boolean doFill(SVGParser.Properties atts, RectF bounding_box) {
            ...
            if (fillString != null) {
                if (fillString.startsWith("url(#")) {
                    ...
                    SVGParser.Gradient g = gradientMap.get(id);
                    Shader shader = null;
                    if (g != null) {
                        shader = g.shader;
                    }
        ...
        private boolean doStroke(SVGParser.Properties atts) {
            if (whiteMode) {
        ...
        private void finishGradients() {
            for(SVGParser.Gradient gradient : gradientMap.values()) {
                if (gradient.xlink != null) {
                    SVGParser.Gradient parent = gradientMap.get(gradient.xlink);
                    if (parent != null) {
        ...
        private void doColor(SVGParser.Properties atts, Integer color, boolean fillMode, Paint paint) {
...
//                    int height = (int) FloatMath.ceil(getFloatAttr("height", atts));
                ...
                SVGParser.NumberParse numbers = getNumberParseAttr("points", atts);
                if (numbers != null) {
                    Path p = new Path();
